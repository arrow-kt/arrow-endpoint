...
class SchemaKtTest :
  StringSpec({
    ...
    "enum derivation" { Schema.reflect<Test>() shouldBe Schema.enum<Test>() }

    "public data class derivation" { Schema.reflect<Person>() shouldBe Schema.person() }

    "sealed class derivation" { Schema.reflect<Sum>() shouldBe Schema.sum() }
  })
...
fun Schema.Companion.sumA(): Schema<SumA> =
  Schema.Product(
    ...
    listOf(Pair(FieldName("test"), enum<Test>()), Pair(FieldName("person"), person()))
  )
...
fun Schema.Companion.sum(): Schema<Sum> =
  Schema.Coproduct(Schema.ObjectInfo(Sum::class.qualifiedName!!), nonEmptyListOf(sumA(), sumB()))
