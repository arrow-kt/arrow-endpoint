...
class UriTest : StringSpec({
  ...
  val testData: List<Pair<String, List<Pair<String, String>>>> = listOf(
    "basic" to listOf(
  ...
  val testDoesNotTrimOtherWhitespaceChars = listOf(
  ...
  for ((groupName, testCases: List<Pair<String, String>>) in testData) {
    for ((i: Int, pair: Pair<String, String>) in testCases.withIndex()) {
      "[$groupName] should interpolate to ${pair.second} (${i + 1})" {
  ...
  "hostname characters" {
    ...
    parseToUri("http://σ")
      .fold({ fail("this should work") }, {
        val host = it.host().toString()
        println("HOST:$host")
        UriCompatibility.encodeDNSHost(host) shouldBe "xn--4xa"
      })
    // TODO: These should be different hosts, but their being encoded as the same
    parseToUri("http://Σ")
      .fold({ fail("this should work") }, {
        val host = it.host().toString()
        println("HOST:$host")
    ...
    parseToUri("http://AB\u00ADCD")
      .fold({ fail("this should work") }, {
        val host = it.host().toString()
        println("HOST:$host")
        UriCompatibility.encodeDNSHost(host) shouldBe "abcd"
      })
    parseToUri("http://\u2121")
      .fold({ fail("this should work") }, {
        val host = it.host().toString()
        println("HOST:$host")
        UriCompatibility.encodeDNSHost(host) shouldBe "tel"
      })
    parseToUri("http://\uD87E\uDE1D").fold({ fail("this should work") }, {
      val host = it.host().toString()
      println("HOST:$host")
  ...
  "hostname ipv6" {
    parseToUri("http://[::1]/")
      .shouldBeRight().host().toString() shouldBe "::1"

    parseToUri("http://[::1]/")
      .shouldBeRight().toString() shouldBe "http://[::1]/"

    parseToUri("http://[::1]:8080/")
      .shouldBeRight().port() shouldBe 8080

    parseToUri("http://user:password@[::1]/")
      .shouldBeRight().authority?.userInfo?.password shouldBe "password"
  ...
  "paths" {
    parseToUri("http://host/%00")
      .shouldBeRight().path() shouldContainExactly listOf("\u0000")
    parseToUri("http://host/a/%E2%98%83/c")
      .shouldBeRight().path() shouldContainExactly listOf("a", "\u2603", "c")
    parseToUri("http://host/a/%F0%9F%8D%A9/c")
      .shouldBeRight().path() shouldContainExactly listOf("a", "\uD83C\uDF69", "c")
    parseToUri("http://host/a/%62/c")
      .shouldBeRight().path() shouldContainExactly listOf("a", "b", "c")
    parseToUri("http://host/a/%7A/c")
      .shouldBeRight().path() shouldContainExactly listOf("a", "z", "c")
    parseToUri("http://host/a/%7a/c")
      .shouldBeRight().path() shouldContainExactly listOf("a", "z", "c")
    parseToUri("http://host/a%f/b")
      .shouldBeLeft().shouldBeTypeOf<UriError.IllegalArgument>()
