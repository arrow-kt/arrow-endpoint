...
/**
 * A transput is either an input, or an output (see:
 * https://ell.stackexchange.com/questions/21405/hypernym-for-input-and-output). The transput
 * interface contain some common functionality, shared by all inputs and outputs.
 *
...
public sealed interface EndpointTransput<A> {
  ...
  public fun <B> mapDecode(f: (A) -> DecodeResult<B>, g: (B) -> A): EndpointTransput<B> =
  ...
  public sealed interface Basic<L, A, CF : CodecFormat> : EndpointTransput<A> {
    ...
    public fun example(example: EndpointIO.Info.Example<A>): Basic<L, A, CF> =
      copyWith(codec, info.example(example))
    public fun examples(examples: List<EndpointIO.Info.Example<A>>): Basic<L, A, CF> =
