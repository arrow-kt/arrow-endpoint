...
public sealed interface InputValueResult {
  public data class Value(val params: Params, val remainingBasicValues: List<Any?>) :
    InputValueResult
  public data class Failure(val input: EndpointInput<*>, val failure: DecodeResult.Failure) :
    InputValueResult

  public companion object {

    /**
     * Returns the value of the input, tupled and mapped as described by the data structure. Values
     * of basic inputs are taken as consecutive values from `values.basicInputsValues`. Hence, these
     * should when (in order).
     */
    public fun from(
      input: EndpointInput<*>,
      values: DecodeBasicInputsResult.Values
    ): InputValueResult = from(input, values.basicInputsValues)

    @Suppress("UNCHECKED_CAST")
    private fun from(input: EndpointInput<*>, remainingBasicValues: List<Any?>): InputValueResult =
      when (input) {
        is EndpointInput.Pair<*, *, *> ->
          handlePair(input.first, input.second, input.combine, remainingBasicValues)
        is EndpointIO.Pair<*, *, *> ->
          handlePair(input.first, input.second, input.combine, remainingBasicValues)
        ...
        is EndpointInput.Basic<*, *, *> ->
          remainingBasicValues.headAndTailOrNull()?.let { (v, valuesTail) ->
            ...
            ?: throw IllegalStateException(
              "Mismatch between basic input values: $remainingBasicValues, and basic inputs in: $input"
            )
      }
      ...
      when (val res = from(left, remainingBasicValues)) {
        is Value ->
          when (val res2 = from(right, res.remainingBasicValues)) {
            is Value -> Value(combine(res.params, res2.params), res2.remainingBasicValues)
            is Failure -> res2
          }
        is Failure -> res
      }
    ...
    private fun <II, T> handleMappedPair(
      ...
      when (val res = from(wrapped, remainingBasicValues)) {
        is Value ->
          when (val res2 = codec.decode(res.params.asAny as II)) {
            is DecodeResult.Value -> Value(Params.ParamsAsAny(res2.value), res.remainingBasicValues)
