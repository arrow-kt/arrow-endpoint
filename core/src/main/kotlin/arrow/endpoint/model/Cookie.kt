...
/**
 * A cookie name-value pair. The `name` and `value` should be already encoded (if necessary), as
 * when serialised, they end up unmodified in the header.
 */
public data class Cookie(val name: String, val value: String) {
  ...
  public companion object {
    ...
    private fun validateName(name: String): String? = Rfc2616.validateToken("Cookie name", name)
    ...
    public fun of(name: String, value: String): Either<String, Cookie> =
      Nullable.zip(validateName(name), validateValue(value)) { a, b -> "$a, $b" }?.left()
        ?: Cookie(name, value).right()
        ...
        s.split(";").map { ss ->
          ...
          when (cookie.size) {
            ...
            2 -> of(cookie[0], cookie[1])
              ...
              throw RuntimeException("Cookie.parse failed. Expected size 1 or 2 but found $cookie")
          }
        }
