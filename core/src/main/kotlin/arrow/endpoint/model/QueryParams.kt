...
public value class QueryParams(internal val ps: List<Pair<String, List<String>>>) {
  ...
  ) : this(map.entries.map { (k, v) -> Pair(k, listOf(v)) })
  ...
  public fun getMulti(s: String): List<String>? = ps.firstOrNull { it.first == s }?.second
  ...
  public fun param(p: Map<String, String>): QueryParams =
    QueryParams(ps + p.map { (k, v) -> Pair(k, listOf(v)) })
}
