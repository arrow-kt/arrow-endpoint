...
/**
 * A [https://en.wikipedia.org/wiki/Uniform_Resource_Identifier URI]. Can represent both relative
 * and absolute URIs, hence in terms of [https://tools.ietf.org/html/rfc3986], this is a URI
 * reference.
 *
 * All components (scheme, host, query, ...) are stored decoded, and become encoded upon
 ...
 *
 * @param querySegments Either key-value pairs, single values, or plain query segments. Key value
 * pairs will be serialized as `k=v`, and blocks of key-value pairs/single values will be combined
 * using `&`. Note that no `&` or other separators are added around plain query segments - if
 * required, they need to be added manually as part of the plain query segment. Custom encoding
 * logic can be provided when creating a segment.
 */
public data class Uri(
  ...
  public companion object {
    ...
    private val schemeSpecificPartPattern =
      Regex(
        "^?(//(?<authority>((?<userinfo>[^/?#]*)@)?(?<host>(\\[[^\\]]*\\]|[^/?#:]*))(:(?<port>[^/?#]*))?))?(?<path>[^?#]*)(\\?(?<query>[^#]*))?(#(?<fragment>.*))?"
      )

    public operator fun invoke(javaUri: URI): Uri? = parse(javaUri.toString()).orNull()

    public operator fun invoke(url: String): Uri? = parse(url).orNull()
    ...
    public fun parse(url: String): Either<UriError, Uri> {
      ...
      val match: MatchResult =
        schemeSpecificPartPattern.matchEntire(schemeSpecificPart)
      ...
      return Uri(
          ...
          authority =
            Authority(
              userInfo =
                getUserInfoOrNull(match, schemeSpecificPart)
                  ?.fold(
                    {
                      return it.left()
                    },
              ...
              hostSegment =
                getHost(match, schemeSpecificPart)
                  .fold(
                    {
                      return it.left()
                    },
              ...
              port =
                getPort(match, schemeSpecificPart, scheme)
                  ?.fold(
                    {
                      return it.left()
                    },
          ...
          pathSegments =
            getPathSegmentsOrEmpty(match, schemeSpecificPart)
              .fold(
                {
                  return it.left()
                },
          ...
          querySegments =
            getQuerySegmentsOrEmpty(match, schemeSpecificPart)
              .fold(
                {
                  return it.left()
                },
          ...
          fragmentSegment =
            getFragmentSegmentOrNull(match, schemeSpecificPart)
              .fold(
                {
                  return it.left()
                },
    ...
    private fun getUserInfoOrNull(
      match: MatchResult,
      schemeSpecificPart: String
    ): Either<UriError, UserInfo>? =
    ...
    private fun getHost(
      match: MatchResult,
      schemeSpecificPart: String
    ): Either<UriError, HostSegment> =
      match.groups["host"]?.range?.let { range ->
        schemeSpecificPart.substring(range).removeSurrounding(prefix = "[", suffix = "]").let {
          host: String ->
    ...
    private fun getPort(
      ...
      match.groups["port"]?.range?.let { range ->
        val port: Int? =
          schemeSpecificPart.substring(range).let {
            when {
              it.isEmpty() -> null
              else -> {
                try {
    ...
    private fun Int.isDefaultPort(scheme: String) =
      when (scheme) {
        "https" -> 443 == this
        else -> 80 == this
      }

    private fun getPathSegmentsOrEmpty(
      match: MatchResult,
      schemeSpecificPart: String
    ): Either<UriError, PathSegments> =
          ...
          match.groups["path"]?.range?.let { range ->
            ...
            when {
              pathPart.isEmpty() -> emptyList()
                ...
                pathPart.removePrefix("/").split("/").map { segment ->
                  segment
                    .decode()
                    .fold(
                      {
        ...
        when (querySegments.contains("&") || querySegments.contains("=")) {
          true -> {
            querySegments
              .split("&")
              .map { querySegment ->
                querySegment.split("=").map {
                  it.decode(plusAsSpace = true)
                    .fold(
                      { e ->
                        return e.left()
                      },
   ...
   */
  public fun userInfo(username: String, password: String): Uri =
    userInfo(UserInfo(username, password))

  /**
   * Replace the user info with username/password combination. Adds an empty host if one is absent,
   * and user info is defined.
   */
  ...
  public fun authority(a: Authority): Uri = this.copy(authority = a)

  public fun addPath(p: String, vararg ps: String): Uri =
    addPathSegments(listOf(PathSegment(p)) + ps.map { PathSegment(it) })

  public fun addPathSegments(ss: List<PathSegment>): Uri =
    copy(pathSegments = pathSegments.addSegments(ss))

  public fun withPath(p: String, vararg ps: String): Uri =
    withPathSegments(listOf(PathSegment(p)) + ps.map { PathSegment(it) })

  public fun withPathSegments(ss: List<PathSegment>): Uri =
    copy(pathSegments = pathSegments.withSegments(ss))

  /**
   * Replace the whole path with the given one. Leading `/` will be removed, if present, and the
   * path will be split into segments on `/`.
   */
  ...
  public fun withParam(k: String, v: String?): Uri =
    v?.let { withParams(listOf(Pair(k, v))) } ?: this
  ...
  public fun addQuerySegment(qf: QuerySegment): Uri =
    this.copy(querySegments = querySegments + listOf(qf))

  //

  /** Replace the fragment. */
  public fun fragment(f: String?): Uri = fragmentSegment(f?.let { FragmentSegment(it) })
    ...
    copy(
      pathSegments =
        when (pathSegments) {
          ...
          is PathSegments.AbsolutePath ->
            PathSegments.AbsolutePath(pathSegments.segments.map { it.encoding(encoding) })
          is PathSegments.RelativePath ->
            PathSegments.RelativePath(pathSegments.segments.map { it.encoding(encoding) })
        }
    )
        ...
        querySegments.map {
          when (it) {
            is QuerySegment.KeyValue ->
              QuerySegment.KeyValue(it.k, it.v, it.keyEncoding, valueEncoding)
  ...
  override fun toString(): String {
    tailrec fun StringBuilder.encodeQuerySegments(
      qss: List<QuerySegment>,
      ...
      when (val headQuerySegment = qss.firstOrNull()) {
        ...
        is QuerySegment.KeyValue -> {
          if (!previousWasPlain) append("&")
          append(headQuerySegment.keyEncoding(headQuerySegment.k))
            .append("=")
...
public sealed interface UriError {
  @JvmInline public value class UnexpectedScheme(public val errorMessage: String) : UriError
  @JvmInline public value class CantParse(public val errorMessage: String) : UriError
  public object InvalidHost : UriError
  public object InvalidPort : UriError
  @JvmInline public value class IllegalArgument(public val errorMessage: String) : UriError
}
...
public sealed class Segment(public open val v: String, public open val encoding: Encoding) {
  public fun encoded(): String = encoding(v)
  public abstract fun encoding(e: Encoding): Segment
}

public data class HostSegment(override val v: String, override val encoding: Encoding = Standard) :
  Segment(v, encoding) {
...
public data class PathSegment(override val v: String, override val encoding: Encoding = Standard) :
  Segment(v, encoding) {

  public companion object {
    public val Standard: Encoding = { it.encode(Rfc3986.PathSegment) }
  }
...
public sealed interface PathSegments {
  ...
  public data class AbsolutePath(override val segments: List<PathSegment>) : PathSegments {
    ...
    override fun toString(): String =
      segments.joinToString(separator = "/", prefix = "/") { it.encoded() }
  }
...
public sealed interface QuerySegment {
  ...
  public companion object {
    ...
    public val All: Encoding = { UriCompatibility.encodeQuery(it, "UTF-8") }

    /**
     * Encodes only the `&` and `=` reserved characters, which are usually used to separate query
     * parameter names and values.
     */
    public val Standard: Encoding = {
      it.encode(
        allowedCharacters = Rfc3986.Query - setOf('&', '='),
        spaceAsPlus = true,
        encodePlus = true
      )
    }
    ...
    /**
     * Encodes only the `&` reserved character, which is usually used to separate query parameter
     * names and values. The '=' sign is allowed in values.
     */
    ...
    /**
     * Doesn't encode any of the reserved characters, leaving intact all characters allowed in the
     * query string as defined by RFC3986.
     */
    public val Relaxed: Encoding = { it.encode(Rfc3986.Query, spaceAsPlus = true) }

    /**
     * Doesn't encode any of the reserved characters, leaving intact all characters allowed in the
     * query string as defined by RFC3986 as well as the characters `[` and `]`. These brackets
     * aren't legal in the query part of the URI, but some servers use them unencoded. See
  ...
  public data class Value(val v: String, val encoding: Encoding = StandardValue) : QuerySegment {
    override fun toString(): String = "Value($v)"
  }

  /**
   * A query fragment which will be inserted into the query, without and preceding or following
   * separators. Allows constructing query strings which are not (only) &-separated key-value pairs.
   *
   * @param encoding How to encode the value, and which characters should be escaped. The RFC3986
   * standard defines that the query can include these special characters, without escaping:
   *
   * ```
   * /?:@-._~!$&()*+,;=
   * ```
   ...
   */
  public data class Plain(val v: String, val encoding: Encoding = StandardValue) : QuerySegment {
...
public data class FragmentSegment(
  ...
  public companion object {
    public val Standard: Encoding = { it.encode(Rfc3986.Fragment) }
