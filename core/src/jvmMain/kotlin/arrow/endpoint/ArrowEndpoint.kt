...
public object ArrowEndpoint {
  ...
  public fun <A> query(
    name: String,
    codec: Codec<List<String>, A, CodecFormat.TextPlain>
  ): EndpointInput.Query<A> = EndpointInput.Query(name, codec, EndpointIO.Info.empty())
  ...
  public fun <A> header(
    name: String,
    codec: Codec<List<String>, A, CodecFormat.TextPlain>
  ): EndpointIO.Header<A> = EndpointIO.Header(name, codec, EndpointIO.Info.empty())

  // TODO add support for Fixed Headers
  //  fun header(h: Header): EndpointIO.FixedHeader<Unit> = EndpointIO.FixedHeader(h,
  // Codec.idPlain(), EndpointIO.Info.empty())
  //  fun header(name: String, value: String): EndpointIO.FixedHeader<Unit> = header(Header(name,
  // value))
  //  fun headers(): EndpointIO.Headers<List<Header>> = EndpointIO.Headers(Codec.idPlain(),
  // EndpointIO.Info.empty())

  public fun <A> cookie(
    name: String,
    codec: Codec<String?, A, CodecFormat.TextPlain>
  ): EndpointInput.Cookie<A> = EndpointInput.Cookie(name, codec, EndpointIO.Info.empty())

  public fun cookies(): EndpointIO.Header<List<Cookie>> = header(Header.Cookie, Codec.cookiesCodec)
  ...
  ): EndpointIO.StringBody<A> = EndpointIO.StringBody(charset, codec, EndpointIO.Info.empty())
  ...
  /**
   * Json codecs are usually derived from json-library-specific implicits. That's why integrations
   * with various json libraries define `jsonBody` methods, which directly require the
   * library-specific implicits.
   *
  ...
  public fun <A> xmlBody(codec: XmlCodec<A>): EndpointIO.StringBody<A> = anyFromStringBody(codec)
  ...
  public fun <A> formBody(
    codec: Codec<String, A, CodecFormat.XWwwFormUrlencoded>
  ): EndpointIO.StringBody<A> = anyFromStringBody(codec)

  public fun <A> formBody(
    charset: Charset,
    codec: Codec<String, A, CodecFormat.XWwwFormUrlencoded>
  ): EndpointIO.StringBody<A> = anyFromStringBody(codec, charset)
  ...
  /**
   * Maps [StatusCode]s to outputs. All outputs must have a common supertype [A]. Typically, the
   * supertype is a sealed class, and the mappings are implementing classes.
   *
   * Note that exhaustiveness of mappings is not checked (that all subtypes of a sealed class are
   * covered).
   *
   * We can use this to for example to define an [EndpointOutput] that returns an [Either] of [Int]
   * or [String]. Depending on if it returns [Either.Left] or [Either.Right] it allows you to return
   * a different [StatusCode].
   *
   * The server decided based on the type, which [StatusCode] it needs to return, and the client
   * determines based on the [StatusCode] which [Codec] to use to decode the response.
   *
   * ```kotlin
   ...
   */
  public fun <A> oneOf(
    firstCase: EndpointOutput.StatusMapping<A>,
    vararg otherCases: EndpointOutput.StatusMapping<A>
  ): EndpointOutput.OneOf<A, A> =
    EndpointOutput.OneOf(firstCase prependTo otherCases.toList(), Codec.idPlain())

  /**
   * Create a status mapping which uses [statusCode] and [output] if the outputted value matches the
   * type of [A]. Should be used in [oneOf] output descriptions.
   */
  public inline fun <reified A> statusMapping(
    statusCode: StatusCode,
    output: EndpointOutput<A>
  ): EndpointOutput.StatusMapping<A> =
