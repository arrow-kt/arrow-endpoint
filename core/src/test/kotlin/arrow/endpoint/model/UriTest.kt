...
class UriTest : FunSpec() {
  ...
  private val testData: List<Pair<String, List<Pair<String, String>>>> =
    listOf(
      "basic" to
        listOf(
      ...
      "authority with parameters" to
        listOf("http://$v1.com?x=$v2" to "http://$v1.com?x=$v2queryEncoded"),
      "ipv4" to
        listOf(
      ...
      "ipv6" to
        listOf(
          ...
          "http://[2001:0abcd:1bcde:2cdef::9f2e:0690:6969]/x" to
            "http://[2001:0abcd:1bcde:2cdef::9f2e:0690:6969]/x",
          "http://[::1]:8080/x" to "http://[::1]:8080/x",
          "http://[2001:0abcd:1bcde:2cdef::9f2e:0690:6969]/x" to
            "http://[2001:0abcd:1bcde:2cdef::9f2e:0690:6969]/x",
          "http://[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:8080" to
      ...
      "path with parameters" to
        listOf(
          "http://example.com/$v1?x=$v2" to "http://example.com/$v1?x=$v2queryEncoded",
          "http://example.com/$v1/$v2?x=$v2" to
            "http://example.com/$v1/$v2encoded?x=$v2queryEncoded"
        ),
      "query parameter values" to
        listOf(
      ...
      "query parameter without value" to
        listOf(
          "http://example.com?$v1" to "http://example.com?$v1",
      ...
      "everything" to
        listOf(
          "http://$v1.$v2.com/$v1/$v2?$v1=$v2#$v1" to
            "http://$v1.$v2encoded.com/$v1/$v2encoded?$v1=$v2queryEncoded#$v1",
      ...
      "encode unicode characters that are encoded as 3+ UTF-8 bytes" to
        listOf(
  ...
  private val testTrimStart =
    listOf(
      "parse trim start Ascii white spaces" to
        listOf(
  ...
  private val testDoesNotTrimOtherWhitespaceChars =
    listOf(
      "parse does not trim other whitespace characters" to
        listOf(
          "http://h/\u000b" to "/%0B",
          "http://h/\u001c" to "/%1C",
          "http://h/\u001d" to "/%1D",
  ...
  init {
    ...
    for ((groupName, testCases: List<Pair<String, String>>) in
      testDoesNotTrimOtherWhitespaceChars) {
      for ((i: Int, pair: Pair<String, String>) in testCases.withIndex()) {
        test("[$groupName] should interpolate to ${pair.second} (${i + 1})") {
    ...
    for ((groupName, testCases: List<Pair<String, String>>) in testScheme) {
      for ((i: Int, pair: Pair<String, String>) in testCases.withIndex()) {
        test("[$groupName] should interpolate to ${pair.second} (${i + 1})") {
          Uri.parse(pair.first)
            .fold(
              { uriError ->
                (uriError as UriError.UnexpectedScheme).errorMessage shouldBe pair.second
              },
    ...
    test("hostname characters") {
      ...
      Uri.parse("http://σ")
        .fold(
          { fail("this should work") },
          { UriCompatibility.encodeDNSHost(it.host().toString()) shouldBe "xn--4xa" }
        )
      Uri.parse("http://Σ")
        .fold(
          { fail("this should work") },
      ...
      Uri.parse("http://\u2121")
        .fold(
          { fail("this should work") },
          { UriCompatibility.encodeDNSHost(it.host().toString()) shouldBe "tel" }
        )
      Uri.parse("http://\uD87E\uDE1D")
        .fold(
          { fail("this should work") },
    ...
    test("hostname ipv6") {
      ...
      Uri.parse("http://[::1]/")
        .fold({ fail("this should work") }, { it.toString() shouldBe "http://[::1]/" })
      Uri.parse("http://[::1]:8080/")
        .fold({ fail("this should work") }, { it.port() shouldBe 8080 })
      Uri.parse("http://user:password@[::1]/")
        .fold(
          { fail("this should work") },
          { it.authority?.userInfo?.password shouldBe "password" }
        )
    ...
    test("port") {
      Uri.parse("http://host:80/")
        .fold({ fail("this should work") }, { it.toString() shouldBe "http://host/" })
      Uri.parse("http://host:99/")
        .fold({ fail("this should work") }, { it.toString() shouldBe "http://host:99/" })
      Uri.parse("http://host:/")
        .fold({ fail("this should work") }, { it.toString() shouldBe "http://host/" })
      Uri.parse("http://host:65535/")
        .fold({ fail("this should work") }, { it.port() shouldBe 65535 })
      Uri.parse("http://host:0/")
        .fold({ it shouldBe UriError.InvalidPort }, { fail("Expecting an error") })
      Uri.parse("http://host:65536/")
        .fold({ it shouldBe UriError.InvalidPort }, { fail("Expecting an error") })
      Uri.parse("http://host:-1/")
        .fold({ it shouldBe UriError.InvalidPort }, { fail("Expecting an error") })
      Uri.parse("http://host:a/")
        .fold({ it shouldBe UriError.InvalidPort }, { fail("Expecting an error") })
      Uri.parse("http://host:%39%39/")
        .fold({ it shouldBe UriError.InvalidPort }, { fail("Expecting an error") })
    }
    ...
    test("paths") {
      ...
      Uri.parse("http://host/a/%E2%98%83/c")
        .fold(
          { fail("this should work") },
          { it.path() shouldContainExactly listOf("a", "\u2603", "c") }
        )
      ...
      Uri.parse("http://host/a/%62/c")
        .fold(
          { fail("this should work") },
          { it.path() shouldContainExactly listOf("a", "b", "c") }
        )
      Uri.parse("http://host/a/%7A/c")
        .fold(
          { fail("this should work") },
          { it.path() shouldContainExactly listOf("a", "z", "c") }
        )
      Uri.parse("http://host/a/%7a/c")
        .fold(
          { fail("this should work") },
      ...
      Uri.parse("http://host/a%f/b")
        .fold(
          { it::class shouldBeSameInstanceAs UriError.IllegalArgument::class },
    ...
    test("[query parameter values] should interpolate correctly") {
      Uri.parse("http://example.com?x=a=b")
        .fold(
          { fail("this should work") },
          {
            it.querySegmentsEncoding(QuerySegment.StandardValue).toString() shouldBe
      ...
      Uri.parse("http://host/?a=!$(),/:;?@[]\\^`{|}~")
        .fold(
          { fail("this should work") },
          {
            it.querySegmentsEncoding(QuerySegment.All).toString() shouldBe
    ...
    test("[fragments] should interpolate correctly") {
      Uri.parse("http://host/#=[]:;\"~|?#@^/$*")
        .fold(
          ...
          { uri ->
            uri
              .fragmentSegmentEncoding { FragmentSegment.RelaxedWithBrackets(it) }
