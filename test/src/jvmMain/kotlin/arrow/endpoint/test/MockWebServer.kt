...
public fun <I, E, O> ServerEndpoint<I, E, O>.toDispatcher(): Dispatcher =
  object : Dispatcher() {
    override fun dispatch(request: RecordedRequest): MockResponse {
      ...
      val interpreter = ServerInterpreter(serverRequest, RequestBody(request), emptyList())

      return runBlocking(Dispatchers.Default) {
        interpreter.invoke(this@toDispatcher)?.let {
          MockResponse().setResponseCode(it.code.code).setBody(it).apply {
  ...
  override suspend fun <R> toRaw(bodyType: EndpointIO.Body<R, *>): R {
    return when (bodyType) {
      ...
      is EndpointIO.ByteBufferBody ->
        withContext(Dispatchers.IO) { ByteBuffer.wrap(ctx.body.readByteArray()) }
      is EndpointIO.InputStreamBody -> ctx.body.inputStream()
      is EndpointIO.StringBody ->
        withContext(Dispatchers.IO) { ctx.body.readByteArray().toString(bodyType.charset) }
    } as
      R
  }
}
...
public fun RecordedRequest.toServerRequest(): ServerRequest =
  ServerRequest(
    ...
    connectionInfo =
      ConnectionInfo(Nullable.zip(requestUrl?.host, requestUrl?.port, ::Address), null, null),
    ...
    queryParameters =
      requestUrl
        ...
        .orEmpty()
        .map { name ->
          Pair(name, requestUrl?.queryParameterValues(name)?.filterNotNull().orEmpty())
        }
        .toList()
        .let(::QueryParams)
  )
