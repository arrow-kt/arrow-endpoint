...
internal fun Map<TSchema.ObjectInfo, String>.referenceOr(codec: Codec<*, *, *>): Referenced<Schema> =
  referenceOr(codec.schema())

internal fun Map<TSchema.ObjectInfo, String>.referenceOr(schema: TSchema<*>): Referenced<Schema> =
  when (schema) {
    is TSchema.List -> when (val element = schema.element) {
      is TSchema.Object -> Referenced.Other(
        Schema(
...
internal fun Map<TSchema.ObjectInfo, String>._referenceOrSchema(schema: TSchema<*>): Referenced<Schema> =
  when (schema) {
    ...
    is TSchema.Number.Int -> Referenced.Other(Schema(type = OpenApiType.integer, format = Format.int32))
    is TSchema.Number.UByte -> Referenced.Other(Schema(type = OpenApiType.integer, format = Format.byte))
    is TSchema.Number.Byte -> Referenced.Other(Schema(type = OpenApiType.integer, format = Format.byte))
    is TSchema.Number.UShort -> Referenced.Other(Schema(type = OpenApiType.integer, format = Format.int32))
    is TSchema.Number.Short -> Referenced.Other(Schema(type = OpenApiType.integer, format = Format.int32))
    is TSchema.Number.Double -> Referenced.Other(Schema(type = OpenApiType.number, format = Format.double))
    is TSchema.Number.Float -> Referenced.Other(Schema(type = OpenApiType.number, format = Format.float))
    is TSchema.Number.Long -> Referenced.Other(Schema(type = OpenApiType.number, format = Format.int64))
    is TSchema.Number.ULong -> Referenced.Other(Schema(type = OpenApiType.number, format = Format.int64))
    ...
    is TSchema.DateTime -> Referenced.Other(Schema(type = OpenApiType.string, format = Format.datetime))

    is TSchema.Product -> Referenced.Other(
      Schema(
    ...
    is TSchema.List -> Referenced.Other(
      Schema(
        type = OpenApiType.array,
        items = when (val element = schema.element) {
    ...
    is TSchema.Coproduct -> Referenced.Other(
      Schema(
        oneOf = schema.schemas.mapNotNull {
          (it as? TSchema.Product)?.objectInfo?.let { info -> Referenced.Ref(getReference(info)) }
        },
        // discriminator =
      )
    )

    is TSchema.Either -> Referenced.Other(
      Schema(
        oneOf = listOf(schema.left, schema.right).mapNotNull {
          (it as? TSchema.Product)?.objectInfo?.let { info -> Referenced.Ref(getReference(info)) }
        }, // discriminator =
      )
    )
    is TSchema.Enum -> Referenced.Other(
      Schema(
        type = OpenApiType.string,
        enum = schema.values.map(TSchema.EnumValue::name)
      )
    )
    is TSchema.Nullable -> _referenceOrSchema(schema.element)
    is TSchema.Map -> Referenced.Other(
      Schema(
        type = OpenApiType.`object`,
        required = emptyList(),
        additionalProperties = when (val v = schema.valueSchema) {
    ...
    is TSchema.OpenProduct -> Referenced.Other(
      Schema(
        type = OpenApiType.`object`,
        required = emptyList(),
        additionalProperties = when (val v = schema.valueSchema) {
