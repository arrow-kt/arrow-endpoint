...
public fun Endpoint<*, *, *>.toOperationResponses(schemas: Map<Schema.ObjectInfo, String>): Responses {
  ...
  return output.toResponse("200", Response(""), schemas) + errorOutput.toResponse("default", null, schemas)
}
...
public fun EndpointOutput<*>.toResponse(
  ...
  return Responses(
    default = map["default"],
    responses = map.let { (it - "default") }.mapKeysTo(linkedMapOf()) { (k, _) -> StatusCode(k.toInt()) }
  )
}
...
private fun outputsToResponse(
  ...
  val headers = outputs.mapNotNull {
    when (it) {
      is EndpointIO.Header -> Pair(
        it.name,
        Referenced.Other(
          Header(
  ...
  val bodies = outputs.mapNotNull {
    when (it) {
      is EndpointIO.Body<*, *> -> Pair(it.info.description, it.codec.toMediaTypeMap(schemas, it.info.examples))
      else -> null
    }
  }
  ...
  val statusCodeDescriptions = outputs.flatMap {
    when (it) {
      is EndpointOutput.StatusCode -> it.documentedCodes.mapNotNull { (code, example) -> if (code == sc) example.description else null }
  ...
  return if (body != null || headers.isNotEmpty()) Response(description, headers = headers.toMap(linkedMapOf()), content = content)
  else if (outputs.isNotEmpty()) Response(description)
  else null
}
