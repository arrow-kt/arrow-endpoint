...
internal fun <A> Iterable<A>.calculateUniqueKeys(toName: (A) -> String): Map<A, String> =
  fold(Assignment<A>(linkedMapOf(), linkedMapOf())) { (nameToT, tToKey), t ->
      ...
      Assignment(nameToT + Pair(key, t), tToKey + Pair(t, key))
    }
    .tToKey
...
/**
 * Keeps only the first object data for each `SObjectInfo`. In case of recursive objects, the first
 * one is the most complete as it contains the built-up structure, unlike subsequent ones, which
 * only represent leaves (Tapir#354).
 */
internal fun Iterable<Pair<Schema.ObjectInfo, Schema<*>>>.unique():
  List<Pair<Schema.ObjectInfo, Schema<*>>> {
...
// mapping to the top-level. In the map, the `None` key stands for the default status code, and a
// `Some` value
// to the status code specified using `statusMapping` or `statusCode(_)`. Any empty outputs are
// skipped.
...
private fun <I> EndpointOutput<I>.asBasicOutputsOrList(): BasicOutputsOrList {
  fun <A> throwMultipleOneOfMappings(): A =
    throw IllegalArgumentException("Multiple one-of mappings in output $this")
  ...
  return when (this) {
    is EndpointOutput.Pair<*, *, *> ->
      mergeMultiple(listOf(this.first.asBasicOutputsOrList(), this.second.asBasicOutputsOrList()))
    is EndpointIO.Pair<*, *, *> ->
      mergeMultiple(listOf(this.first.asBasicOutputsOrList(), this.second.asBasicOutputsOrList()))
