...
): OpenApi = toOpenAPI(Info(title, version = version), options)

public fun <I, E, O> Endpoint<I, E, O>.toOpenAPI(
  info: Info,
  options: OpenAPIDocsOptions = OpenAPIDocsOptions()
): OpenApi = listOf(this).toOpenAPI(info, options)
...
public fun Iterable<Endpoint<*, *, *>>.toOpenAPI(
  ...
  return map(pathCreator::pathItem).fold(openApi) { acc, (path, pathItem) ->
    acc.addPathItem(path, pathItem)
  }
}

private fun Map<String, Referenced<Schema>>.toComponents(
/*securitySchemes: SecuritySchemes*/ ): Components? =
  if (this.isNotEmpty() /* || securitySchemes.isNotEmpty() */)
    Components(this /* securitySchemes */)
  else null
