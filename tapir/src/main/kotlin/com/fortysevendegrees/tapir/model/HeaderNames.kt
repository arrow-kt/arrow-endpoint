...
import com.fortysevendegrees.tapir.map
...
object HeaderNames {
  ...
  val ContentHeaders: Set<String> =
    setOf(ContentLength, ContentType, ContentMd5).map(String::toLowerCase)

  val SensitiveHeaders: Set<String> =
    setOf(Authorization, Cookie, SetCookie).map(String::toLowerCase)
