...
import com.fortysevendegrees.tapir.Validator.Single.*
import com.fortysevendegrees.tapir.Validator.Single.Primitive.Max
import com.fortysevendegrees.tapir.Validator.Single.Primitive.MaxLength
import com.fortysevendegrees.tapir.Validator.Single.Primitive.MaxSize
import com.fortysevendegrees.tapir.Validator.Single.Primitive.Min
import com.fortysevendegrees.tapir.Validator.Single.Primitive.MinLength
import com.fortysevendegrees.tapir.Validator.Single.Primitive.MinSize
import com.fortysevendegrees.tapir.Validator.Single.Primitive.Pattern
...
//  fun show: Option[String] = com.fortysevendegrees.tapir.Validator.show(this)
...
//  fun openProduct[V](elemValidator: com.fortysevendegrees.tapir.Validator[V]): com.fortysevendegrees.tapir.Validator[Map[String, V]] =
...
//object com.fortysevendegrees.tapir.Validator extends ValidatorEnumMacro {
...
//
//  case class OpenProduct[E](elementValidator: com.fortysevendegrees.tapir.Validator[E]) extends Single[Map[String, E]] {
//    override fun validate(t: Map[String, E]): List[com.fortysevendegrees.tapir.ValidationError[_]] = {
//    t.flatMap { case(name, value) => elementValidator.validate(value).com.fortysevendegrees.tapir.map(_.prependPath(com.fortysevendegrees.tapir.FieldName(name, name))) }
//  }.toList
//  }
...
//   */
//  case class Ref<A>(private var _wrapped: com.fortysevendegrees.tapir.Validator<A>) extends com.fortysevendegrees.tapir.Validator<A> {
//    fun set(w: com.fortysevendegrees.tapir.Validator<A>): this.type = {
//    if (_wrapped == null) _wrapped =
//      w else throw new IllegalArgumentException (s"com.fortysevendegrees.tapir.Validator reference is already set to ${_wrapped}!")
//    this
//  }
//    fun wrapped: com.fortysevendegrees.tapir.Validator<A> = {
//      if (_wrapped == null) throw new IllegalStateException ("No recursive validator reference set!")
//      _wrapped
//    }
//
//    override fun validate(t: T): List[com.fortysevendegrees.tapir.ValidationError[_]] = wrapped.validate(t)
//  }
//  object Ref {
...
//  //
//
//  fun show<A>(v: com.fortysevendegrees.tapir.Validator<A>): Option[String] = {
//    v match {
...
//      case CollectionElements (el, _) => show(el).com.fortysevendegrees.tapir.map(se => s"elements($se)")
//      case Product (fields) =>
//      fields.flatMap {
//        case(n, f) =>
//        show(f.validator).com.fortysevendegrees.tapir.map(n -> _)
//      }.toList match {
//        case Nil => None
//          case l => Some (l.com.fortysevendegrees.tapir.map { case(n, s) => s"$n->($s)" }.mkString(","))
//      }
//      case c @ Coproduct(_) =>
//      c.subtypes.flatMap {
//        case(n, v) =>
//        show(v).com.fortysevendegrees.tapir.map(n -> _)
//      }.toList match {
//        case Nil => None
//          case l => Some (l.com.fortysevendegrees.tapir.map { case(n, s) => s"$n->($s)" }.mkString(","))
//      }
//      case OpenProduct (el)    => show(el).com.fortysevendegrees.tapir.map(se => s"elements($se)")
